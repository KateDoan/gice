...
MicroSim <- function(v.M_1, v.age, n.i, n.t, v.n, X = NULL, d.c, d.e, TR.out = TRUE, TS.out = FALSE, Trt = FALSE, seed = 1) {
  ...
  # Counter vectors
  cN <- rep(1, n.i)
  cE <- cC <- cD <- cQ <- rep(0, n.i)
  cNEonly <- cNEC <- cNED <- cNC <- cND <- rep(FALSE, n.i)
  ...
  for (t in 1:n.t) { # t <- 3
    ...
    cNEC[m.M[,t+1] == "C" & cC==0 & cD==0 & cE>=1] <- TRUE
    cNED[m.M[,t+1] == "D" & cC==0 & cD==0 & cE>=1] <- TRUE
    cNC[m.M[,t+1] == "C" & cC==0 & cD==0 & cE==0] <- TRUE
    cND[m.M[,t+1] == "D" & cC==0 & cD==0 & cE==0] <- TRUE
    cN[m.M[,t+1] == "N"] = cN[m.M[,t+1] == "N"] + 1
    cE[m.M[,t+1] == "E"] = cE[m.M[,t+1] == "E"] + 1
    cC[m.M[,t+1] == "C"] = cC[m.M[,t+1] == "C"] + 1
    cD[m.M[,t+1] == "D"] = cD[m.M[,t+1] == "D"] + 1
    cQ[m.M[,t+1] == "Q"] = cQ[m.M[,t+1] == "Q"] + 1
  } # close the loop for the time points 
  cNEonly[cC==0 & cD==0 & cE>=1] <- TRUE
  ...
  results <- list(TR = TR,
                  numNEonly = sum(cNEonly), numNEC = sum(cNEC), numNED = sum(cNED), numNC = sum(cNC), numND = sum(cND),
                  pNEonly = sum(cNEonly)/n.i, pNEC = sum(cNEC)/n.i, pNED = sum(cNED)/n.i, pNC = sum(cNC)/n.i, pND = sum(cND)/n.i) # store the results from the simulation in a list  
