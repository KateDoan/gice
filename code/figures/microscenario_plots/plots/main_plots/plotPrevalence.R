plotPrevEdited <- function(chosen, chosen_leg, ALPHAS, df_prop,
                     g, ylab, ylim, text = "NEVER USERS", leg = FALSE) {
  df_prop <- df_prop %>% filter(scenario %in% chosen)
  p <- ggplot(df_prop,aes_string(x="year", y=g, group="scenario",
                                  col="scenario", shape="scenario", 
                                  linetype="scenario", fill="scenario",
                                  alpha="scenario")) +
    theme_classic() +
    ...
    scale_linetype_manual(values = LINETYPES,
                          name = " ",
                          limits = unique(chosen_leg),
                          labels = LABELS) +
    scale_shape_manual(values = SHAPES,
                       name = " ",
                       limits = unique(chosen_leg),
                       labels = LABELS) +
    scale_fill_manual(values = FILZ,
                      name = " ",
                      limits = chosen_leg,
                      labels = LABELS) +
