...
getPlotsEdited <- function(chosen, df_prop, df_LYL){
  ...
  pCandDleg <- plotPrevEdited(chosen, COMBS, ALPHAS1, df_prop, "CandD", ylab="Prevalence of Cigarette and Dual Users (%)", ylim = c(0, 15.5), text = " ", leg=TRUE)
  pCDEleg <- plotPrevEdited(chosen, COMBS, ALPHAS1, df_prop, "CDE", ylab="Prevalence of e/Cigarette Users (%)", ylim = c(0, 16.5), text = " ", leg=TRUE)
  ...
  pD1 <- plotPrevEdited(chosen, SINGLES, ALPHAS2, df_prop, "D", ylab, ylim = c(0, 5), text = "DUAL USERS", leg=TRUE)
  pD2 <- plotPrevEdited(chosen, COMBS, ALPHAS2, df_prop, "D", ylab, ylim = c(0, 5), text = "DUAL USERS", leg=TRUE)
  pD <- plotPrevEdited(chosen, chosen, ALPHAS1, df_prop, "D", ylab, ylim = c(0, 5), text = "DUAL USERS")
  pE <- plotPrevEdited(chosen, chosen, ALPHAS1, df_prop, "E", ylab, ylim = c(0, 5), text = "E-CIGARETTE ONLY USERS")
  ...
  ####
  plotsSet <- list(pN=pN, pC=pC, pQ=pQ, pD1=pD1, pD2=pD2, pD=pD, pE=pE,
                   pNandQ=pNandQ, pCandD=pCandD, pCDE=pCDE, 
                   pCandDleg=pCandDleg, pCDEleg=pCDEleg,
                   pAnnualLYL=pAnnualLYL, pAnnualLYLdis=pAnnualLYLdis,
                   pCandDleg1=pCandDleg1, pCandDleg2=pCandDleg2, pCandDleg3=pCandDleg3)
