...
snapshotDList <- colSums(numDArray[idxStartTrunc:idxEndTrunc,])
snapshotEList <- colSums(numEArray[idxStartTrunc:idxEndTrunc,])
...
PD_D <- ((deathRateD - deathRateN) * numDArray)[idxStartTrunc:idxEndTrunc,]
PD_E <- ((deathRateE - deathRateN) * numEArray)[idxStartTrunc:idxEndTrunc,]
PDsum <- PD_C + PD_Q + PD_D + PD_E
PD <- colSums(PDsum)
cumsumPD <- cumsum(PD)
