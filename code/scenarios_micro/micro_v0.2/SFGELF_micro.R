...
ELFAge <- 18
big <- "JP"
outDir <- file.path("code", "scenarios_micro", "micro_v0.2", "outData")
#############################################################################################################################
...
a.ecig <- 0.01        # initiation rate of e-cig increases by 1% annually
...
ageInitCig <- 17
SFGBirth <- 2000 
ageInitCigIdx <- ageInitCig - startAge + 1
tStartSFG <- SFGBirth + ageInitCig - startYear + 1
d.c <- 0.03           # initiation rate of cigarette decreases 3% per year during the duration of SFG
...
Probs <- function(M_it, v.index, v.RR, t) { 
  ...
  m.p.it[,M_it == "N"] <- rbind(v.NN.ni[M_it=="N"], v.NC.ni[M_it=="N"], 0, v.ND.ni[M_it=="N"], v.NE.ni[M_it=="N"], v.toX.ni[M_it=="N"])   # transition probabilities when never smoke
  ...
  m.p.it[,M_it == "E"] <- rbind(v.EN.ni[M_it=="E"], v.EC.ni[M_it=="E"], 0, v.ED.ni[M_it=="E"], v.EE.ni[M_it=="E"], v.toX.ni[M_it=="E"])
